...
export default function AboutPage({ params }: { params: Promise<{ lng: Locale }> }) {
    ...
    const trustLinks = [
        {
            href: '/privacy',
            title: t('legal.links.privacy.label'),
            description: t('legal.links.privacy.description'),
        },
        {
            href: '/terms',
            title: t('legal.links.terms.label'),
            description: t('legal.links.terms.description'),
        },
        {
            href: '/contact',
            title: t('legal.links.contact.label'),
    ...
    return(
        <>
            ...
            <div className='w-full flex flex-col items-center justify-center scroll-y-auto px-4 pb-10 text-primary'>
                <div className='w-full max-w-6xl h-full flex flex-col gap-4 p-2 lg:flex-row'>
                    <div className='flex flex-1 flex-col gap-4'>
                    <div className='flex flex-col rounded-[1.75rem] border border-grey bg-background/95 p-5 shadow-[0_1rem_2rem_rgba(0,0,0,0.06)]'>
                    ...
                    <div className='flex flex-col rounded-[1.75rem] border border-grey bg-background/95 p-5 shadow-[0_1rem_2rem_rgba(0,0,0,0.06)]'>
                        ...
                        <Link
                            href={`/${lng}/contact`}
                            className='mt-4 inline-flex w-fit rounded-full border border-primary px-4 py-2 text-sm font-semibold uppercase tracking-[0.2em] transition hover:border-accent hover:text-accent'
                        >
                            {t('about.open-contact-page')}
                        </Link>
                    </div>
                    <div className='flex flex-col rounded-[1.75rem] border border-grey bg-background/95 p-5 shadow-[0_1rem_2rem_rgba(0,0,0,0.06)]'>
                    ...
                    <aside className='flex w-full flex-col gap-4 lg:w-[22rem]'>
                        <div className='rounded-[1.75rem] border border-grey bg-background/95 p-5 shadow-[0_1rem_2rem_rgba(0,0,0,0.06)]'>
                            <p className='text-xs font-semibold uppercase tracking-[0.28em] text-accent'>{t('about.trust-centre')}</p>
                            <h2 className='mt-2 font-nunito text-3xl'>{t('about.site-policies-and-contact')}</h2>
                            <p className='mt-3 leading-7 text-primary/80'>
                                {t('about.trust-centre-info')}
                            </p>
                        </div>
                        {trustLinks.map((link) => (
                            <Link
                                key={link.href}
                                href={`/${lng}${link.href}`}
                                className='rounded-[1.75rem] border border-grey bg-white/80 p-5 transition duration-300 hover:-translate-y-1 hover:border-accent hover:shadow-[0_1rem_2rem_rgba(0,0,0,0.08)]'
                            >
                                <p className='font-nunito text-2xl'>{link.title}</p>
